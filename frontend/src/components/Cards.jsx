...
const cs={
  card:{background:"#0f1624",border:"1px solid #162033",borderRadius:8,padding:12,cursor:"pointer",
...
export function IndexCard({data,meta,loc,selected,onClick}){
  ...
  useEffect(()=>{
    if(priceRef.current&&prevLtp.current&&prevLtp.current!==ltp){
  ...
  return(
    <div style={{...cs.card,borderColor:selected?"#ffc94d":"#162033",
      ...
      <div style={cs.price} ref={priceRef}>{fmt(ltp)}</div>
      <div style={{...cs.chg,color:dir==="up"?"#00e676":"#ff3d5a"}}>{arr(p)} {absn(p)}%</div>
      <div style={cs.bar}><div style={{...cs.fill,width:fp+"%",background:dir==="up"?"#00e676":"#ff3d5a"}}/></div>
      {loc?.zone&&<div style={cs.loc}>
        <span style={{color:"#4a5568"}}>BOP:{fmt(loc.bop,0)}</span>
        <span style={{...cs.zone,background:loc.zone==="CALL"?"rgba(0,230,118,.15)":loc.zone==="PUT"?"rgba(255,61,90,.15)":"rgba(255,201,77,.1)",
...
export function StockCard({data,meta,loc,selected,onClick}){
  const ltpc=data?.ltpc||{};const ltp=ltpc.ltp||0,cp=ltpc.cp||ltp
  const p=pct(ltp,cp);const dir=sign(p);const zone=loc?.zone||""
  const zoneBg=zone==="CALL"?"rgba(0,230,118,.05)":zone==="PUT"?"rgba(255,61,90,.05)":"#0f1624"
  return(
    <div style={{...cs.card,background:zoneBg,borderColor:selected?"#ffc94d":zone==="CALL"?"rgba(0,230,118,.15)":zone==="PUT"?"rgba(255,61,90,.15)":"#162033"}}
      onClick={onClick}
      onMouseEnter={e=>e.currentTarget.style.transform="translateY(-1px)"}
      onMouseLeave={e=>e.currentTarget.style.transform=""}>
      {zone&&<div style={{position:"absolute",top:5,right:5,fontSize:7,padding:"1px 4px",borderRadius:2,fontFamily:"'JetBrains Mono',monospace",
        background:zone==="CALL"?"rgba(0,230,118,.15)":"rgba(255,61,90,.15)",color:zone==="CALL"?"#00e676":"#ff3d5a"}}>{zone}</div>}
      <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:10,fontWeight:700,marginBottom:4}}>{meta.n}</div>
      <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:13,fontWeight:700}}>{fmt(ltp)}</div>
      <div style={{fontSize:9,fontFamily:"'JetBrains Mono',monospace",marginTop:1,color:dir==="up"?"#00e676":"#ff3d5a"}}>{arr(p)} {absn(p)}%</div>
    </div>
  )
}
